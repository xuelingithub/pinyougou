       ...
       if(id==null){
        ...
        goodsService.findOne(id).success(
			function(response){
				...
				editor.html($scope.entity.goodsDesc.introduction);/*商品介绍  富文本显示*/
				//图片json转成string
...
//判断规格和规格选项是否被勾选
    $scope.checkAttributeValue=function (specName,optionName) {
        ...
        var object  = $scope.searchObjectByKey(item,'attributeName',specName);//key是string
        if(object!=null){
            if(object.attributeValue.indexOf(optionName)>=0){//查询到规格选项
                return true;
            }else {
                return false;
            }
